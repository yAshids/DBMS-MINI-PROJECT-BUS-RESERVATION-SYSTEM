...
CREATE PROCEDURE GetBookingsByRouteID(IN routeID INT)
BEGIN
    SELECT 
        b.id AS BookingID,
        bd.seat_no AS SeatNumber,
        bd.journey_date AS JourneyDate,
        bd.booking_date AS BookingDate,
        bd.number_of_seats AS NumberOfSeats,
        bd.route_rent AS RouteRent,
        ...
        p.current_phone_no AS CurrentPhoneNo,
        bd.bus_type AS BusType
    FROM 
        bookings b
    JOIN 
        book_detail bd ON b.route_id = bd.route_id
    JOIN 
        payment p ON p.customer_id = b.customer_id
    JOIN 
        customer c ON c.customer_id = p.customer_id
    JOIN 
        route_detail r ON r.route_id = b.route_id
    WHERE 
        b.route_id = routeID;
END //
...
DELIMITER //
mysql> CREATE PROCEDURE ViewTicket (IN customer_id INT)
    -> BEGIN
    ->     SELECT b.id, b.route_id, b.seat_number, b.journey_date,
    ->            c.customer_name, c.email, r.bus_type
    ->     FROM bookings b
    ->     JOIN customer c ON b.customer_id = c.customer_id
    ->     JOIN route_detail r ON b.route_id = r.route_id
...
CREATE PROCEDURE GetBookedBusTickets()
BEGIN
    SELECT DISTINCT
        c.customer_id,
        c.username,
